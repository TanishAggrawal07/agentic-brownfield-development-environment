...
class ImpactService:
    ...
    def ensure_graph_ready(self, project_root: str) -> None:
        """Ensure the dependency graph is built and matches the current analysis."""
        self.project_path = str(Path(project_root).resolve())
        
        # Force build search index if search_service has no APIs
        if not search_service.apis or search_service.project_path != self.project_path:
            logger.info("Search service index is stale or empty. Building index...")
            search_service.build_index(self.project_path)

        # Check if analysis_manager has active data
        if not analysis_manager.files:
            logger.info("Analysis manager files are empty. Loading analysis cache...")
        ...
        # Rebuild graph if the analysis timestamp has changed
        current_analyzed_at = analysis_manager.analyzed_at
        if current_analyzed_at != self._last_analyzed_at or not self.file_dependents:
            logger.info("Rebuilding dependency graph from analysis knowledge model...")
