...
class AIService:
    ...
    either an LLM API (Gemini or OpenAI) or a detailed local structural template.
    """
    ...
    def explain(
        ...
        # 2. Check for LLM Keys and fetch explanation
        explanation = None
        if self.gemini_key:
            explanation = self._call_gemini(resolved_scope, resolved_target, context_data)
        elif self.openai_key:
            explanation = self._call_openai(resolved_scope, resolved_target, context_data)
    ...
    def _call_gemini(self, scope: str, target: str, context: Dict[str, Any]) -> Optional[str]:
        """Request explanation from Gemini API using raw urllib."""
        ...
        prompt = self._build_prompt_text(scope, target, context)
        payload = {
            "contents": [
                {
                    "parts": [{"text": prompt}]
    ...
    def _call_openai(self, scope: str, target: str, context: Dict[str, Any]) -> Optional[str]:
        """Request explanation from OpenAI API using raw urllib."""
        self.openai_key = os.environ.get("OPENAI_API_KEY")
        ...
        payload = {
            "model": "gpt-4o-mini",
            "messages": [
                {"role": "system", "content": "You are Antigravity, an AI assistant integrated into a development environment. Write clear, technical, developer-oriented Markdown explanations of codebase elements based on the parsed metadata context provided."},
