...
class SearchService:
    ...
    def build_index(self, project_root: str) -> None:
        ...
        if not analysis_manager.files:
            # Force analysis loading or check cache directly
    ...
    def search(self, query: str, project_root: str) -> Dict[str, Any]:
        ...
        self.project_path = str(Path(project_root).resolve())
        if not self._indexed or not self.apis:
            self.build_index(self.project_path)

        # 1. Execute Local Scorer to fetch candidate hits
        candidates = self._run_local_search(query)

        # 2. Check for LLM keys to perform semantic candidate ranking
        self.gemini_key = os.environ.get("GEMINI_API_KEY")
        self.openai_key = os.environ.get("OPENAI_API_KEY")
        
        if (self.gemini_key or self.openai_key) and candidates:
            # Re-rank using LLM
    ...
    def _call_llm_rerank(self, query: str, candidates: List[Dict[str, Any]]) -> Optional[List[Dict[str, Any]]]:
        """Send candidate hits to Gemini/OpenAI to re-rank and augment them semantically."""
        # Prompt construction
        prompt = (
        ...
        if self.gemini_key:
            ...
            try:
                ...
                logger.error(f"Gemini search re-rank failed: {exc}")
            ...
            payload = {
                "model": "gpt-4o-mini",
                "messages": [
                    {"role": "system", "content": "You are a code search ranker. Evaluate candidate matches for queries and output a structured JSON array."},
            ...
            try:
                ...
                logger.error(f"OpenAI search re-rank failed: {exc}")
